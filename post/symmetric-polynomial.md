...
ただし$\lambda = (\lambda_1,\ldots,\lambda_N)$は分割で、非減少非負整数列で表される:$\lambda_1\ge \lambda_2\ge \cdots\ge \lambda_N \ge 0.$長さが$N$未満の分割についても、ゼロ埋めによって長さ$N$の分割と見なして同様に定義される。長さが$N$より大きい分割については$0$と定義される。分母は（符号を除いて）Vandermonde行列式であり、差積として表される:
